...
**Goal:** Build a `jh` Python CLI in a new sibling repo at `~/code/github/yo61/jobhound/`, replacing the Taskfile-driven workflow with action-based subcommands that mutate a TOML-backed, git-tracked data root under XDG paths.

**Architecture:** Cyclopts-based CLI in package `jobhound`. Each subcommand is a function in `src/jobhound/commands/<verb>.py`, registered in `cli.py`. Data lives at `~/.local/share/jh/` (XDG default) as a git repo of per-opportunity TOML files + sibling artefacts. Every mutating subcommand validates, writes, and auto-commits unless disabled. High-input verbs prompt interactively via `questionary` when flags are missing.

**Tech stack:** Python 3.13, `uv`, `cyclopts`, `questionary`, `tomllib` (stdlib) + `tomli-w`, `pytest`, `ruff`, `ty`, `prek`. (Originally specified `typer`; migrated to `cyclopts` on 2026-05-11.)
...
> **Note on snippets in Tasks 15–24.** The code blocks below were mechanically
> migrated from typer to cyclopts on 2026-05-11 (commits `ac8d346`, `096159d`,
> `fe2ceea`). The transformations applied: `typer.Option`/`typer.Argument`
> wrappers stripped; `typer.echo(msg, err=True)` → `print(msg, file=sys.stderr)`;
> `typer.Exit(N)` → `SystemExit(N)`; `CliRunner().invoke(app, ...)` → an
> `invoke` fixture in `conftest.py`. Treat the snippets as a directional guide,
> not copy-paste source — they may need:
>
> - `import sys`, `from typing import Annotated`, `from cyclopts import Parameter`
>   added to the import block.
> - `/` after the positional `slug_query` and `*,` before the first option to
>   make options keyword-only (see committed `commands/apply.py` for the shape).
> - `questionary` usage where the original called `typer.confirm` / `typer.prompt`.
>
> Use Tasks 11–14 (already committed in cyclopts) as the style reference.
...
def _seed(tmp_jh, tmp_path, target_status: str, invoke) -> None:
...
def test_withdraw_from_active(tmp_jh, invoke, tmp_path) -> None:
    _seed(tmp_jh, invoke, tmp_path, "applied")
...
def test_ghost_from_active(tmp_jh, invoke, tmp_path) -> None:
    _seed(tmp_jh, invoke, tmp_path, "applied")
...
def test_accept_only_from_offer(tmp_jh, invoke, tmp_path) -> None:
    _seed(tmp_jh, invoke, tmp_path, "applied")
...
def test_accept_from_offer(tmp_jh, invoke, tmp_path) -> None:
    _seed(tmp_jh, invoke, tmp_path, "offer")
    result = invoke(["accept", "foo", "--today", "2026-05-15"])
    assert result.exit_code == 0, result.output
    opp = read_meta(tmp_jh.db_path / "opportunities" / "2026-05-foo-em" / "meta.toml")
...
def test_decline_from_offer(tmp_jh, invoke, tmp_path) -> None:
    _seed(tmp_jh, invoke, tmp_path, "offer")
...
def test_delete_requires_yes_or_confirm(tmp_jh, invoke) -> None:
    _seed(tmp_jh, invoke)
    result = invoke(["delete", "foo"], input="n\n")
    assert result.exit_code != 0
...
def run() -> None:
    ...
    if not remotes.stdout.strip():
        print("no remote configured; set one with `git -C <db> remote add origin <url>`", file=sys.stderr)
        raise SystemExit(1)
    result = subprocess.run(
    ...
    if result.returncode != 0:
        ...
        raise SystemExit(code=result.returncode)
    print("pushed.")
```
...
def test_build_writes_outputs_to_cache(tmp_jh, invoke) -> None:
    _seed(tmp_jh, invoke)
    result = invoke(["build", "--today", "2026-05-11"])
    assert result.exit_code == 0, result.output
...
def test_today_prints_due_items(tmp_jh, invoke) -> None:
    _seed(tmp_jh, invoke)
    result = invoke(["today", "--today", "2026-05-11"])
    assert result.exit_code == 0, result.output
...
def run(
    ...
    print(f"built into {paths.cache_dir}")
```
...
def run(today: Annotated[str | None, Parameter(show=False)] = None) -> None:
    """Build dashboards and print today's digest."""
    run_build(today=today)
    cfg = load_config()
    paths = paths_from_config(cfg)
    print((paths.cache_dir / "today.md").read_text())
```
...
app.command(cmd_build.run, name="build")
app.command(cmd_today.run, name="today")
app.command(cmd_dashboard.run, name="dashboard")
```
...
def test_pdf_renders_a_file(tmp_jh, invoke, tmp_path) -> None:
    md = tmp_path / "doc.md"
    md.write_text("# Hello\n\nThis is a test document.\n")
    result = invoke(["pdf", "--file", str(md)])
    assert result.exit_code == 0, result.output
...
def test_cv_requires_slug_with_cv_md(tmp_jh, invoke) -> None:
    invoke(["new", "--company", "Foo", "--role", "EM", "--today", "2026-05-01"])
    opp_dir = tmp_jh.db_path / "opportunities" / "2026-05-foo-em"
    (opp_dir / "cv.md").write_text("# CV\n\nExperience: lots.\n")
    result = invoke(["cv", "foo"])
    assert result.exit_code == 0, result.output
...
def run(slug_query: str) -> None:
    ...
    cv_md = opp_dir / "cv.md"
    if not cv_md.exists():
        print(f"{cv_md} does not exist", file=sys.stderr)
        raise SystemExit(1)
...
app.command(cmd_pdf.run, name="pdf")
app.command(cmd_cv.run, name="cv")
```

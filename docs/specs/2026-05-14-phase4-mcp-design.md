...
The MCP server is **another adapter**, sibling to `cli.py` and
`commands/`. It contains no domain logic — only protocol translation. It
lives in a new top-level subpackage:
...
src/jobhound/
  ...
  application/                        # unchanged (Phase 3a)
  commands/                           # unchanged (Phase 3a)
  ...
  mcp/                                # NEW adapter subpackage
    ...
    tools/                            # one module per logical group
      __init__.py
      reads.py                        # 5 tools
      lifecycle.py                    # 7 tools (state transitions)
      fields.py                       # 12 tools (replaces jh edit)
      relations.py                    # 4 tools (tags/contacts/links)
      ops.py                          # 4 tools (notes/archive/delete/sync)
```
...
```
client → fields.set_priority(slug="acme", level="high")
       → slug    = resolve_slug("acme", paths.opportunities_dir)
       → opp_pre = repo.find(slug)
       → opp_post= opp_pre.with_priority(Priority("high"))
       → repo.save(opp_post, opp_dir, message="set_priority: <slug> high")
       → snap_post = build_snapshot(opp_post, opp_dir, archived=False,
                                     today=date.today())
       → changed   = compute_diff(opp_pre, opp_post)
       → return {opportunity: snapshot_to_dict(snap_post), changed}
```
...
| Layer | Approach |
|---|---|
| Domain (`OpportunityQuery`, `Repository`, `Opportunity.apply()`, …) | **Already tested in Phase 3a (201 tests). Not re-tested.** |
| Pure helpers (`compute_diff`, `tool_error_response`) | Unit tests on the pure functions. No fixtures. |
| Each tool function | Direct function call with the standard fixture. Verifies argument parsing, domain dispatch, response shape. |
...
tests/mcp/
  ...
  test_tools_lifecycle.py     # new/apply/log/withdraw/ghost/accept/decline
  test_tools_fields.py        # 12 setters + touch
  test_tools_relations.py     # add_tag/remove_tag/add_contact/set_link
  test_tools_ops.py           # add_note/archive/delete/sync
...
Roughly **45–55 new tests** total. Most are tight one-assertion happy
paths or error-mapping checks. The `call_tool` fixture keeps each test
~5 lines.

Aim is *coverage of the adapter contract*, not exhaustive enumeration of
inputs — the domain layer already does the latter.

...
def test_show_opportunity_returns_show_envelope(
    mcp_paths: Paths,
    repo: OpportunityRepository,
) -> None:
    payload = json.loads(show_opportunity(repo, "acme"))
    assert "opportunity" in payload
    assert payload["opportunity"]["slug"] == "2026-05-acme-em"


def test_show_opportunity_unknown_slug_returns_error(
    mcp_paths: Paths,
    repo: OpportunityRepository,
) -> None:
    payload = json.loads(show_opportunity(repo, "nonexistent"))
    assert payload["error"]["code"] == "slug_not_found"


def test_show_stats(mcp_paths: Paths, repo: OpportunityRepository) -> None:
    payload = json.loads(show_stats(repo))
...
def test_show_stats_returns_error_on_corrupt_meta(
    mcp_paths: Paths,
    repo: OpportunityRepository,
) -> None:
    bad = mcp_paths.opportunities_dir / "2026-05-acme-em" / "meta.toml"
